...
class BaseDataset(data.Dataset):
    ...
    def processing_val(self, image, input_h, input_w):
        image = cv2.resize(image, (input_w, input_h))
        out_image = image.astype(np.float32) / 255.
        out_image = out_image - 0.5
        out_image = out_image.transpose(2, 0, 1).reshape(1, 3, input_h, input_w)
    ...
    def __getitem__(self, index):
        ...
        if self.phase == 'test' or self.phase == 'val2' or self.phase == 'train2':
        ...
        #elif self.phase == 'val':
        #    img_id = self.img_ids[index]
        #    image = self.processing_val(image, self.input_h, self.input_w)
        #    return {'image': image, 
        #            'img_id': img_id, 
        #            'image_w': image_w,
        #            'image_h': image_h} 
